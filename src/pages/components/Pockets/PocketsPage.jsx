        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/30 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
                ...
                <div className="flex justify-center gap-4">
                    <button onClick={onClose} className="flex-1 py-2 px-4 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50">
                        Batal
                    </button>
                    <button onClick={onConfirm} className="flex-1 py-2 px-4 bg-red-600 text-white font-medium rounded-lg hover:bg-red-700">
                        Ya, Hapus
                    </button>
                </div>
            </div>
...
const MobileSkeleton = () => (
  <div className="animate-pulse">
    <header className="flex flex-row justify-between items-center mb-6 gap-4">
        <div><div className="h-7 bg-slate-200 rounded w-32"></div></div>
        <div className="h-10 w-10 bg-slate-200 rounded-full"></div>
    </header>
    <div className="bg-white rounded-2xl shadow-sm p-4 mb-6">
        ...
        <div className="space-y-3">
            {[...Array(2)].map((_, i) => (<div key={i} className="p-4 bg-slate-100 rounded-lg h-12"></div>))}
        </div>
    </div>
    <div className="bg-white rounded-2xl shadow-sm p-4">
        ...
        <div className="flex flex-col items-center">
            ...
            <div className="w-full max-w-xs space-y-2">
                {[...Array(2)].map((_, i) => (<div key={i} className="h-6 w-full bg-slate-100 rounded-md"></div>))}
            </div>
        </div>
    </div>
  </div>
...
const PocketsPage = () => {
  ...
  const [confirmModalState, setConfirmModalState] = useState({ isOpen: false, pocketId: null });

  const navigate = useNavigate();

  const refetchAllPocketData = async () => {
    // Tidak set loading di sini agar refresh latar belakang lebih mulus
    setError(null);
    try {
      ...
      setLoading(false); // Set loading false hanya setelah semua proses selesai
    }
  };
  ...
  const pieChartData = {
    ...
    datasets: [{
        ...
        borderColor: "#ffffff", borderWidth: 6, borderRadius: 8, hoverOffset: 8,
    }],
  };
  ...
  const handleCloseModal = () => { setIsPocketModalOpen(false); setEditingPocket(null); };
              ...
              <div className="space-y-3 h-full overflow-y-auto pr-2 custom-scrollbar">
                {accounts.length > 0 ? (
                  accounts.map((account, index) => {
                    ...
                    return (
                      <div key={account.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-3.5 bg-gray-50 rounded-lg border border-gray-100 gap-3">
                        <div className="flex items-center min-w-0">
                          <div className="p-2 rounded-lg mr-3 flex-shrink-0" style={{ backgroundColor: `${color}20` }}>
                            <IconComponent size={20} style={{ color }} />
                          </div>
                          <span className="font-medium text-gray-800 truncate text-base">{account.name}</span>
                        </div>
                        <div className="flex items-center space-x-2 flex-shrink-0 self-end sm:self-center">
                          <span className="font-semibold text-gray-900 text-base">{formatCurrency(account.currentBalance)}</span>
                          <button onClick={() => handleOpenEditModal(account)} className="p-2 text-gray-500 hover:bg-gray-200 rounded-full transition-colors">
                            <Pen size={16} />
                          </button>
                          <button onClick={() => handleOpenDeleteConfirm(account.id)} className="p-2 text-red-500 hover:bg-red-100 rounded-full transition-colors">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </div>
                  ...
                  <div className="text-center py-16 text-gray-500">
                    ...
                    <button onClick={handleOpenAddModal} className="text-primary font-semibold mt-1">Buat baru?</button>
                  </div>
                )}
              </div>
            </div>
                ...
                <div className="w-full max-w-xs flex-1 min-h-0">
                  <div className="h-full overflow-y-auto custom-scrollbar pr-2">
                    <div className="flex flex-wrap justify-center gap-3">
                      {accounts.sort((a, b) => b.currentBalance - a.currentBalance).map((account, idx) => (
